...
std::unique_ptr<RenderFactoryImpl> GlobalManager::m_renderFactory;
...
void GlobalManager::Startup(const std::wstring& strResourcePath,
	...
	RenderFactoryType renderType)
{
	m_dwUiThreadId = GetCurrentThreadId();

	m_renderFactory = std::make_unique<RenderFactoryImpl>();
	bool ret = m_renderFactory->Initialize(renderType);
	ASSERT(ret);
	if(!ret)
		return;
...
dui::common::dui_refptr<dui::render::IRenderContext> GlobalManager::CreateRenderContext()
{
	ASSERT(m_renderFactory);
	return m_renderFactory->CreateRenderContext();
}

dui::common::dui_refptr<dui::render::IPen> GlobalManager::CreatePen(DWORD color, int width /*= 1*/)
{
	ASSERT(m_renderFactory);
	return m_renderFactory->CreatePen(color, width);
}

dui::common::dui_refptr<dui::render::IBrush> GlobalManager::CreateBrush(DWORD color)
{
	ASSERT(m_renderFactory);
	return m_renderFactory->CreateBrush(color);
}

dui::common::dui_refptr<dui::render::IMatrix> GlobalManager::CreateMatrix()
{
	ASSERT(m_renderFactory);
	return m_renderFactory->CreateMatrix();
}

dui::common::dui_refptr<dui::render::IPath> GlobalManager::CreatePath()
{
	ASSERT(m_renderFactory);
	return m_renderFactory->CreatePath();
}
